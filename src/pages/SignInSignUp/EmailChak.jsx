...
export default function EmailChak() {
  ...
  const from = location?.state?.from?.pathname || "/login";
    ...
    if (email===emailDb.email) {
      ...
      toast.success(" have a user")

     const  sentPasswordEmail= async()=>{
  ...
  return (
            ...
            <div className="grid gap-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" id="email" name="email" className="py-3 px-4 block w-full border-2 border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm" required />


                </div>
                <p className="hidden text-xs text-red-600 mt-2" id="email-error">Please include a valid email address so we can get back to you</p>
              </div>
              <button type="submit" className="py-3 px-4 inline-flex justify-center 
              items-center gap-2 rounded-md border border-transparent font-semibold
               bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2
                ...
                text-sm dark:focus:ring-offset-gray-800">Reset password</button>
            </div>
          </form>
        </div>
      </div>
    ...
    {/* <div>
        <a className="btn btn-secondary hover:bg-blue-600" href="/forgotPassword">
            forgat page
        </a>
    </div> */}
